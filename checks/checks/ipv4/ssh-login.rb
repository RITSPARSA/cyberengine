...
@services = @cyberengine.services('SSH Login','ipv4','ssh')
@defaults = @cyberengine.defaults('SSH Login','ipv4','ssh')
...
def build_request(service,address)
  ...
  # Command
  command = service.properties.random('command') || @defaults.properties.random('command')
    ...
    @services.each do |service|
      service.properties.addresses.each do |address|
        ...
        begin
          ...
          # Get request output
          response = execute_request(request,service,@defaults)
